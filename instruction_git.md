...
Для того, чтобы переключаться между ветками и выбирать какую-то конкретную ветку, нужно воспользоваться следующей командой:
...
Удалённые репозитории нужны для... (дополнить для чего они нужны)
